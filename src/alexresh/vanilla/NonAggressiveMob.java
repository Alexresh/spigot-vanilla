...
public class NonAggressiveMob implements Listener {
    ...
    public void mobTargetPlayer(EntityTargetEvent event){
        if(!config.getBoolean("nonAgressiveMobModule.mobsDoNotAttackThePlayerWithTheirHead"))
            return;
    ...
    public void onBadMobSpawn(EntitySpawnEvent event){
        if((event.getEntityType().equals(EntityType.BAT))&&(config.getBoolean("nonAgressiveMobModule.disableBatSpawn"))){
            event.setCancelled(true);
            return;
        }
        if((event.getEntityType().equals(EntityType.PHANTOM))&&(config.getBoolean("nonAgressiveMobModule.disablePhantomSpawn"))){
            event.setCancelled(true);
        }
    }
    ...
    public void mobDeath(EntityDeathEvent event){
        if(event.getEntityType() == EntityType.CREEPER){
            Random r = new Random();
            if(r.nextInt(100)+1<=config.getInt("nonAgressiveMobModule.creeperDropHead.chance")){
                event.getDrops().add(new ItemStack(Material.CREEPER_HEAD));
            }
        }
        if(event.getEntityType() == EntityType.ZOMBIE){
            Random r = new Random();
            if(r.nextInt(100)+1<=config.getInt("nonAgressiveMobModule.zombieDropHead.chance")){
                event.getDrops().add(new ItemStack(Material.ZOMBIE_HEAD));
            }
        }
        if(event.getEntityType() == EntityType.SKELETON){
            Random r = new Random();
            if(r.nextInt(100)+1<config.getInt("nonAgressiveMobModule.skeletonDropHead.chance")){
                event.getDrops().add(new ItemStack(Material.SKELETON_SKULL));
            }
        }
        if(event.getEntityType() == EntityType.PLAYER){
            Random r = new Random();
            if(r.nextInt(100)+1<config.getInt("nonAgressiveMobModule.playerDropHead.chance")){
